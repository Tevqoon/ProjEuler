...
(defun all-the-same (list)
  (cond ((or (null list) (null (cdr list)))
         t)
        ((not (member (car list) (cdr list) :test #'equal))
         nil)
        (t
         (all-the-same (cdr list)))))

(defun forms-straight-p (vals)
  (if (null (set-exclusive-or vals '(2 3 4 5 14)))
      t
      (let* ((min (reduce #'min vals))
             (max (+ min 4)))
        (loop for i from min to max
              if (not (member i vals))
                return nil
              when (= i max)
                return t))))
...
(defun counts (vals)
  (let ((h (make-hash-table :test #'equal)))
    (loop for i in vals
          ...
          do (incf (gethash i h)))
    (loop for key being each hash-key of h
          collect (gethash key h))))

(defun sort-by-occurences (list)
  (let ((h (make-hash-table :test #'equal)))
    (loop for i in list
          if (not (gethash i h))
            do (setf (gethash i h) 0)
          do (incf (gethash i h)))
    (let ((keys (sort (loop for key being each hash-key of h
                            collect key)
                      (lambda (x y)
                        (if (= (gethash x h)
                               (gethash y h))
      ...
      (loop for i in keys
               append
              (loop for j from 1 to (gethash i h)
                     collect i)))))
...
(defun hand-strength (hand)
  (let* ((suits (suits hand))
        (vals (sort (vals hand) #'>))
        (counts (counts vals)))
    (cond ((and (forms-straight-p vals) (all-the-same suits))
           (cons 9 vals))
          ((member 4 counts)
           (cons 8 (sort-by-occurences vals)))
          ((and (member 3 counts) (member 2 counts))
           (cons 7 (sort-by-occurences vals)))
          ((all-the-same suits)
           (cons 6 vals))
          ((forms-straight-p vals)
           (cons 5 vals))
          ((member 3 counts)
           (cons 4 (sort-by-occurences vals)))
          ((member 2 (cdr (member 2 counts)))
           (cons 3 (sort-by-occurences vals)))
          ((member 2 counts)
           (cons 2 (sort-by-occurences vals)))
          (t
           (cons 1 vals)))))

(defun compare-hands (hand1 hand2)
  (let ((s1 (hand-strength hand1))
        (s2 (hand-strength hand2)))
    (labels ((iterator (h1 h2)
               (cond ((< (car h1) (car h2))
                      2)
                     ((> (car h1) (car h2))
                      1)
                     (t
                      (iterator (cdr h1) (cdr h2))))))
      (iterator s1 s2))))

(print (loop for i in *hands*
             if (= 1 (apply #'compare-hands i))
               sum 1))
